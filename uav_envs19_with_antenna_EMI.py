...
class UAVEnv(object):
    uav_h = 50  # 无人机高度    100unit,each 10meters
    v_uav = 5  # 无人机飞行速度
    square_length = 100
    uav_range = 20
    c = 3e8  # 光速
    ...
    sigma = 10 ** ((-90 - 30) / 10)  # 加性高斯白噪声功率谱密度
    beta = 0.8  # 历史影响因子
    pt_emi = 10 ** (2 / 10)  # 3db
    a = 9.61
    b = 0.28
    ...
    def step(self, action):
        ...
        finish_reward = 0
        connect_reward = 0
        x_u_pr, y_u_pr, c_pr, sinr_max_pr = self.state
        ...
        corresponding_values = sinr[nonzero_indices]  # 获取arr2中与arr1非零元素位置对应的值
        if  len(corresponding_values)==0:
            sinr_max = 0
        else:
            sinr_max = np.max(corresponding_values)  # 找到corresponding_values中的最大值
        c = np.sum(self.connect)
        if c == self.N_U:
            done = True
            finish_reward = 100
        self.state = x_u, y_u, c, sinr_max
        count_reward = (c - c_pr)

        reward = 0.8 * count_reward + 0.05 * out_reward + 0.05 * finish_reward + 0.1 * step_reward
        # print(reward)
        # print(self.state[ :2])
