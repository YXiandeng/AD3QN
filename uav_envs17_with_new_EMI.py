...
class UAVEnv(object):
    ...
    def step(self, action):
        ...
        mean = 50
        std = 20
        EMI_origin = np.exp(-((x_u - mean) ** 2 + (y_u - mean) ** 2) / (2 * std ** 2))
        x = np.linspace(0, 100, 101)
        y = np.linspace(0, 100, 101)
        Z = np.exp(-((x - mean) ** 2 + (y - mean) ** 2) / (2 * std ** 2))
        EMI = (EMI_origin - np.min(Z)) * (self.target_max - self.target_min) / (np.max(Z) - np.min(Z)) + self.target_min
